...
def makeRead(
    ...
    while 0 >= length > genomeLen or length < minReadLength or length > maxReadLength:
    ...
    if circularGenome:
        ...
        offset = int(uniform(-(length - 1) + minReadLength, genomeLen - minReadLength))
...
if __name__ == "__main__":
    ...
    if meanLength > genomeLen:
        raise ValueError(
            "The mean read length (%d) is greater than the "
    ...
    for i in range(args.count):
        ...
        if verbose:
            print(
                "Created read of length %d with %d mutations"
                % (len(read), len(mutationOffsets)),
                file=sys.stderr,
            )
