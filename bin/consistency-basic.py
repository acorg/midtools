...
def plotConsistency(
    ...
    for index in range(len(significantOffsets) - 1):
        ...
        readsCoveringBoth = readsAtOffset[offset1] & readsAtOffset[offset2]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--title", help="The title plot")
    ...
    print(
        "Read %d aligned reads. Found %d significant locations."
        % (len(alignedReads), len(significantOffsets))
    )

    title = args.title or (
        "ARI and AMI at %d significant locations (%s)."
        % (len(significantOffsets), "jitter added" if args.jitter else "no jitter")
    )
