...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rate", type=float, required=True, help="The per-base mutation rate to use"
    )
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Add this string to the end of the read ids. This is added "
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            'Add "-mutations:N" to the end of each read id, where N '
    ...
    for read in reads:
        ...
        if verbose:
            print(
                "%d mutation%s made in read (len %d) %s"
                % (count, s(count), len(read), read.id),
                file=sys.stderr,
            )
        read.id = read.id + (("-mutations:%d" % count) if editIds else "") + idSuffix
